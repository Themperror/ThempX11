...
namespace Themp
{
	...
	void ShadowPCF::DrawLight()
	{
		D3D* l_D3D = D3D::s_D3D;
		l_D3D->m_DevCon->RSSetState(l_D3D->m_RasterizerState);
		l_D3D->m_DevCon->PSSetShader(m_PCFLightingMaterial->m_PixelShader, 0, 0);
		l_D3D->m_DevCon->VSSetShader(m_PCFLightingMaterial->m_VertexShader, 0, 0);
		...
		l_D3D->m_DevCon->PSSetSamplers(0, 4, m_PCFLightingMaterial->m_SamplerStates);
		...
		l_D3D->m_DevCon->IASetInputLayout(m_PCFLightingMaterial->m_InputLayout);
	...
	void ShadowPCF::DrawPointShadow()
	{
		D3D* l_D3D = D3D::s_D3D;
		Game* l_Game = Themp::System::tSys->m_Game;
		l_D3D->m_DevCon->RSSetState(l_D3D->m_RasterizerState);
	...
	void ShadowPCF::RenderShadowsPointCamera(PointLight* l)
	{
		m_ShadowCamera->isDirty = true;
		m_ShadowCamera->SetProjection(Camera::CameraType::Perspective);
	...
	void ShadowPCF::RenderShadowsSpotCamera(SpotLight* l)
	{
		m_ShadowCamera->SetPosition(XMFLOAT3(l->position.x, l->position.y, l->position.z));
		m_ShadowCamera->SetProjection(Camera::CameraType::Perspective);
	...
	void ShadowPCF::RenderShadowsDirectionalCamera(DirectionalLight* l)
	{
		m_ShadowCamera->SetPosition(l->position.x, l->position.y, l->position.z);
